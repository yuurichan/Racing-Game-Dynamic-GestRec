...
current_model_path = os.path.join(dir_path, "models", "_7-5f-lstm_model_7.keras")
...
def main():
    ...
    while True:
        ...
        1. Append landmarks to pred_seq
        2. Trim pred_seq for it to be ==5 ([-5:])
        3. Predict with [-5:] when len(pred_seq) == 5
        I think this takes a bit of time to process since:
        One, we have to trim before we predict.
        Two, pred_seq needs to have len==5, which kinda worked because
        - Before trim: len=6
        - After trim: len=5
        ...
        1. Append landmarks to pred_seq
        3. Predict with [-5:] when len(pred_seq) >= 5
        3. Trim pred_seq with [-5:]
        Moving predict up first, then we trim to prevent overflowing.
        EDIT: This doesn't change anything, but atleast we stopped the ==5 thing, which I feel kind of forced.
        - Before trim: len=6
        - Predict
        - After trim: len=5
        '''
        # Append keypoint array constantly (until we reach at least 5 frames for prediction, since len() >= 5)
        ...
        '''
        # This might also be one of the reasons why our predictions were having delays.
        I'm intending to do a couple of tests on this, if it doesn't work then I'll make it
        == 5 and return the pred_seq = pred_seq[-5/-10:] to the upper part
        '''
        # print("Seq: ", pred_sequence[-3:], "---\n")
        # Take the last 5 frames for prediction, if we have 0-6 7 8 then it'd take 4 5 6 7 8 for prediction
        if len(pred_sequence) >= 5:     # At this point ==5 was put on purpose
            res_idx = gest_clsf(pred_sequence[-5:])  # This is our classifier object

        # Get the last 5 frames to prevent pred_sequence from overflowing
        # pred_sequence = pred_sequence[-5:]
        # print("After trimming: ", len(pred_sequence))
